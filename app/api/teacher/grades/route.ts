...
export async function GET() {
  ...
  try {
    ...
    console.error("Teacher grades fetch error:", error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { studentId, subjectId, sectionId, gradingPeriod, score, remarks, academicYear } = await request.json();

    if (!studentId || !subjectId || !sectionId || !gradingPeriod || score === undefined) {
      return NextResponse.json({ message: "studentId, subjectId, sectionId, gradingPeriod, and score are required" }, { status: 400 });
    }
    ...
    console.error("Teacher grade save error:", error);
